	...
	$.jstree.core.prototype = {
		...
		redraw_node : function (node, deep, is_callback, force_render) {
			...
			if(this.settings.core.force_text) {
				...
				var icon = '';
				if(this.settings.core.side_menu){
					var div_id = this.settings.core.tree_div_id;
					if(obj.id == 'all'){
						icon = "<a name='information_menu_group_addGroup_btn' style='border-radius:4px;' onclick='javascript:sidemenu(\""+div_id+"\", \"add\",  \"\",  \"\");'><i class=\"icon-plus\"></i></a>";
					} else if(obj.id == 'NONE'){
						console.log(obj.id);
					} else {
						if(obj.data == undefined || obj.data == null) {
							obj.data = {};
							obj.data.userCount = 0;
							obj.data.callKind = 'newData';
						}
						icon = "<a name='information_menu_group_addGroup_btn' class='tree-btn' style='border-radius:4px;' onclick='javascript:sidemenu(\""+div_id+"\", \"add\", \""+obj.id+"\", \""+obj.data.count+"\");'><i class=\"icon-plus\"></i></a>";
						icon += "&nbsp;&nbsp;<a name='information_menu_group_editGroup_btn' class='tree-btn' style='border-radius:4px;' onclick='javascript:sidemenu(\""+div_id+"\", \"modify\", \""+obj.id+"\", \""+obj.data.count+"\");'><i class=\"icon-edit\"></i></a>";
						icon += "&nbsp;&nbsp;<a name='information_menu_group_deleteGroup_btn' class='tree-btn' style='border-radius:4px;' onclick='javascript:sidemenu(\""+div_id+"\", \"remove\", \""+obj.id+"\", \""+obj.data.count+"\");'><i class=\"icon-remove\"></i></a>";
					
						icon += '&nbsp;&nbsp;<a href="#" id="settingGroup_'+obj.id+'" name="settingGroup_'+obj.id+'" onclick="javascript:settingGroup(\''+obj.id+'\')"><i class="icon-cog"></i></a>';
						//icon += '&nbsp;&nbsp;<span><label class="topcoat-checkbox" style="vertical-align:middle;"><input type="checkbox" onclick="updateGroupView(\''+obj.id+'\')"> <div class="topcoat-checkbox__checkmark"></div> 대시보드 그룹만 출력</label></span>';
					}
				}
				
				if(obj.icon == 'img/tree_icon.png') {
					icon = '';
				}
				var txt = '<span style="display:inline-block;">' + obj.text+'</span>&nbsp;&nbsp;&nbsp;<div id="r_' + obj.id + '" style="display: inline;">'+icon+'</div>';
				node.childNodes[1].innerHTML += txt;
			}
 ...
 */
	...
	$.jstree.plugins.massload = function (options, parent) {
		this.init = function (el, options) {F
...
/**
 * 트리 구성 시, 노드옆에 메뉴 기능 수행
 * core.side_menu = true일 경우, 메뉴 노출
 *
 * @param action
 */
function sidemenu(div_id, action, id, userCount){
	var tree = $('#'+div_id).jstree(true);
	var text = "조직명";
	...
	if(action == 'add'){
		...
		var node = { id: newId, text: text, icon: "img/tree_icon_open.png"};
		$node = $('#'+div_id).jstree('create_node', parent, node, 'last');
		tree.edit($node);
		$('#r_'+id).css("display", "none");
	} else if(action == 'modify'){
		$node = tree.get_node({id: id});
		tree.edit($node);
	} else if(action == 'remove'){
		if(!userCount || userCount == 0 || userCount == undefined || userCount == 'undefined') {
			$node = tree.get_node({id: id});
			tree.delete_node($node);
		} else {
			alertBox0('하위 조직 및 사용자가 등록되어 있는 조직 입니다. 삭제할 수 없습니다.');
		}
	} else if(action == 'userplus') {
		location.href = "userRegist.jsp?groupId="+id;
	}
}

//조직 생성시 사용되는 UUID 생성기
function guid() {
	function s4() {
